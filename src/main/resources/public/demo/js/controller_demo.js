...
Fox.DemoCtrl = function($routeParams, $scope, $http) {
    ...
    var
        ...
        nif_anchorOf = 'http://persistence.uni-leipzig.org/nlp2rdf/ontologies/nif-core#anchorOf';

    var searchCallback = function(turtle) {
                ...
                store.execute(query, function(err, results) {
                    if (!err) {
                        for (var i = 0; i < results.length; i++) {
                            //console.log(results[i].s.token +'/'+results[i].s.value);
                            store.node(results[i].s.value, function(err, graph) {
                                var triples = graph.triples;
                                for (i = 0; i < triples.length; i++) {
                                    if (triples[i].predicate.nominalValue == nif_anchorOf) {
                                        console.log(triples[i].object.nominalValue);
                                    }
                                }
                            });
                        }
