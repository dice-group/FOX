...
public class DetailCSV {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Instances oracel = new DataSource(folder + "/training.arff").getDataSet();
        for (int run = 1; run <= runs; run++) {
            init();
            lastRun = run + "";

            for (int i = 1; i <= 10; i++) {
                Instances part = new DataSource(folder + "/classified_" + run + "_" + i + ".arff").getDataSet();
                lastFold = i + "";
                // all instances for one fold
                for (int ii = 0; ii < part.numInstances(); ii++) {
                    String classifiedClass = part.instance(ii).stringValue(classindex);
                    String oracelclasss = oracel.instance(currentRow).stringValue(classindex);
                    if (oracelclasss.equals(EntityClassMap.N))
                        nullRows++;
                    currentRow++;

                    //
                    if (lastOracleClass.equals(oracelclasss) && !oracelclasss.equals(EntityClassMap.N) && (ii < part.numInstances() - 1)) {
                        classifiedClasses.add(classifiedClass);
                    } else {

                        boolean correct = true;
                        for (String c : classifiedClasses) {
        ...
        String filename = folder + "/" + alg + ".csv";
