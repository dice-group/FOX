/*
 * Balie - BAseLine Information Extraction Copyright (C) 2004-2007 David Nadeau
 *
 ...
 * NERF (Named Entity Recognition Framework) is an semi-supervised NER system made of generated
 * Named Entity lists and disambiguation rules. <br>
...
public class NamedEntityRecognitionNerfEXT extends NamedEntityRecognition {
  ...
  private final DisambiguationRulesI m_Rules;
  private final PriorCorrectionI m_Prior;
  private final NamedEntityTypeEnumI[] m_Mapping;
  private final boolean m_FullDebug;

  private final Hashtable<String, ArrayList<Integer>> m_CapWord2Index; // all
  // indexes of
  // capitalized
  // form of a
  // word

  private final Hashtable<String, UnlistedWordInfo> m_UnlistedWords; // lookup of
  // words that
  // are part of
  // entities but
  // are not
  ...
  private class UnlistedWordInfo {
    public UnlistedWordInfo(final NamedEntityType pi_Type, final int pi_Group) {
      m_Type = pi_Type;
    ...
    private final NamedEntityType m_Type;
    private final int m_AliasGroup;
  }

  public NamedEntityRecognitionNerfEXT(final TokenList pi_TokenList,
      final LexiconOnDiskI pi_Lexicon, final DisambiguationRulesI pi_Rules,
      final PriorCorrectionI pi_Prior, final NamedEntityTypeEnumI[] pi_Mapping,
      final boolean pi_FullDebug) {
    ...
    m_CapWord2Index = new Hashtable<>();
    m_UnlistedWords = new Hashtable<>();
  }

  @Override
  protected void ApplyDisambiguation() {
    map = new HashMap<>();
    final NamedEntityTypeEnumI[] intermediateTagSet = m_TokenList.NETagSet();
  ...
  private void CheckEntityNounAmbiguityNerf() {
    ...
    final Hashtable<String, ArrayList<Integer>> hLowercased = new Hashtable<>(); // all
    // indexes
    // of
    // a
    // lowercased
    ...
    // a
    // word
    final Hashtable<String, ArrayList<Integer>> hAmbiguous = new Hashtable<>(); // all
    // indexes
    // of
    // an
    // ambiguous
    ...
    for (int i = 0; i != m_TokenList.Size(); ++i) {
      final Token tok = m_TokenList.Get(i);
      ...
      if (bLastTokenIsQuote || tok.IsSentenceStart() || bInsideAllCapitalized) {
        if (TokenFeature.Feature.StartsWithCapital.Mechanism().GetBooleanValue(tok.Features())
            && TokenConsts.Is(tok.Type(), TokenConsts.TYPE_WORD)) {
    ...
    final ArrayList<Integer> alRejectedBothCases = new ArrayList<>();
    final ArrayList<Integer> alRejectedAlwaysLower = new ArrayList<>();
    final ArrayList<Integer> alRejectedAlwaysAmbiguous = new ArrayList<>();

    final Enumeration<String> eCur = m_CapWord2Index.keys();
    while (eCur.hasMoreElements()) {
      final String strCur = eCur.nextElement();

      // invalidate word that appears both lowercased and uppercased
      if (hLowercased.containsKey(strCur)) {
        alRejectedBothCases.addAll(m_CapWord2Index.get(strCur));
        alRejectedBothCases.addAll(hLowercased.get(strCur));
      }

      // invalidate word that appears only in ambiguous pos
      else if (hAmbiguous.containsKey(strCur)) {
        final ArrayList<Integer> alCap = m_CapWord2Index.get(strCur);
        final ArrayList<Integer> alAmb = hAmbiguous.get(strCur);
    ...
    final Enumeration<String> enumL = hLowercased.keys();
    while (enumL.hasMoreElements()) {
      final String strCur = enumL.nextElement();
      // invalidate word that appears only lowercased
      if (!m_CapWord2Index.containsKey(strCur)) {
        alRejectedAlwaysLower.addAll(hLowercased.get(strCur));
      }
    }
    ...
    while (iCur.hasNext()) {
      final int nIdx = iCur.next().intValue();
      final Token tok = m_TokenList.Get(nIdx);
      final NamedEntityType net = tok.EntityType();

      if (NERServices.NaturallyLowercased(net)) {
        ForceEntityTypeExt(nIdx,
            NamedEntityType.Intersection(net, NamedEntityTypeEnum.NaturallyOccursLowercased(),
                new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.NATURAL_LOWERCSE,
                    new Object[] {})));
      } else if (NERServices.OneWordEntity(net)) {
        ForceEntityTypeExt(nIdx,
            new NamedEntityType(NamedEntityTypeEnum.NOTHING, m_TagSetSize,
                new NamedEntityExplanation(
                    NamedEntityExplanation.ExplanationType.REJECTED_LOWERCASED_AND_UPPERCASED,
                    new Object[] {})));
        if (m_FullDebug) {
    ...
    while (iCur.hasNext()) {
      ...
      if (NERServices.NaturallyLowercased(net)) {
        ...
        ForceEntityTypeExt(nIdx,
            new NamedEntityType(NamedEntityTypeEnum.NOTHING, m_TagSetSize,
                new NamedEntityExplanation(
                    NamedEntityExplanation.ExplanationType.REJECTED_LOWERCASED_OR_AMBIGUOUS,
                    new Object[] {})));
        if (m_FullDebug) {
  ...
  private void PreCheckEntityBoundaryNerf() {
    ...
    for (int i = 0; i != m_TokenList.Size() - 1; ++i) {
      final Token curTok = m_TokenList.Get(i);
      final Token nextTok = m_TokenList.Get(i + 1);
      ...
      if (curTok.SentenceNumber() == nextTok.SentenceNumber() && curTok.EntityType().IsEnd()) {
        ...
        if (nextTok.EntityType().IsStart()) {
          ...
          if (curTok.EntityType().GetLabel(m_TokenList.NETagSet()) != null
            ...
            final Hashtable<String, NamedEntityTypeEnumI> hSharedLabs = NERServices
                .SharedLabel(curTok.EntityType(), nextTok.EntityType(), m_TokenList.NETagSet());

            if (hSharedLabs.size() > 0) {
              final NamedEntityType forceType = NERServices.Labels2Types(hSharedLabs, m_TagSetSize);
        ...
        // a period or a ampersand follows.
          ...
          final Token secondNextTok = m_TokenList.Get(i + 2);
          if (secondNextTok.EntityType().IsStart()
            ...
            final Hashtable<String, NamedEntityTypeEnumI> hSharedLabs = NERServices.SharedLabel(
                curTok.EntityType(), secondNextTok.EntityType(), m_TokenList.NETagSet());
  ...
  private void CheckEntityEntityAmbiguityNerf() {
    ...
    final NamedEntityAliasResolution near =
        new NamedEntityAliasResolution(m_TokenList, m_TagSetSize, m_DesiredEntities);
    ...
    for (int i = 0; i != m_TokenList.Size() - 1; ++i) {
      ...
      if (NamedEntityAliasResolution.ValidAliasToken(curTok)) {
        final NamedEntityType curType =
            NamedEntityType.Intersection(curTok.EntityType(), m_DesiredEntities, null);

        // check all aliases for a cue
        final Integer nGroupId = curTok.NamedEntityAlias();
        final NamedEntityAlias neAlias = near.GetAlias(nGroupId);

        if (!neAlias.DominantType().HasNoTag() && !neAlias.DominantType().equals(curType)) {
          final NamedEntityType net = neAlias.DominantType()
              .clone(new NamedEntityExplanation(
                  NamedEntityExplanation.ExplanationType.ALIAS_DOMINANT_TYPE,
                  new Object[] {neAlias.DominantType().GetLabel(m_TokenList.NETagSet())}));
          if (curTok.EntityType().HasNoTag()) {
            PromoteUnknownWordToEntity(curTok, net);
            if (m_FullDebug) {
              System.out.println("Force type: " + curTok.Raw() + " --> "
                  + neAlias.DominantType().GetLabel(m_TokenList.NETagSet()));
            }
          } else {
  ...
  private void ApplyEntityEntityClassifiersNerf() {
    ...
    for (int i = 0; i != m_TokenList.Size() - 1; ++i) {
      ...
      if (curTok.EntityType().IsStart()) {
        final NamedEntityType curTypes =
            NamedEntityType.Intersection(curTok.EntityType(), m_DesiredEntities, null);
        ...
        if (curTypes.TypeCount() > 1) {
          ...
          final String[] strTypes = new String[curTypes.TypeCount()];
          int idx = 0;
          final Hashtable<String, NamedEntityTypeEnumI> hLabel2NEType = new Hashtable<>();
          if (m_FullDebug) {
            System.out.print("(");
          }

          final NamedEntityTypeEnumI[] NETagSet = m_TokenList.NETagSet();
          ...
          final int nStart = i;
          final int nEnd = FindEndIndexNerf(nStart);
          final String[] strCtx = new String[m_Rules.ContextSize() * 2];
          // left
          int nSlot = 0;
          ...
          if (netBefore != null && curTypes.Intersect(netBefore)) {
            ForceEntityTypeExt(i,
                NamedEntityType.Intersection(curTypes, netBefore, new NamedEntityExplanation(
                    NamedEntityExplanation.ExplanationType.ADJACENT_SHARE_TYPE, new Object[] {})));
            if (m_FullDebug) {
              System.out.println("--> merged with preceeding of same type");
            }
          } else if (netAfter != null && curTypes.Intersect(netAfter)) {
            ForceEntityTypeExt(i,
                NamedEntityType.Intersection(curTypes, netAfter, new NamedEntityExplanation(
                    NamedEntityExplanation.ExplanationType.ADJACENT_SHARE_TYPE, new Object[] {})));
            if (m_FullDebug) {
            ...
            // may remove that now that prior are known!!!!!
            if (hLabel2NEType.containsKey("month")) {
              final Hashtable<String, Double> hPriorMap = new Hashtable<>(); // bidon
              ForceEntityTypeExt(i,
                  new NamedEntityType(hLabel2NEType.get("month"), NETagSet.length,
                      new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.CLASSIFIER,
                          new Object[] {curTok.Raw(), hLabel2NEType.get("month").Label(), 0.9,
                              hPriorMap})));
              if (m_FullDebug) {
              ...
              final double[] fPrior =
                  m_Prior.GetPrior(curTok.LexiconMatch(), strTypes[0], strTypes[1]);

              // keep a prior map for explanations
              final Hashtable<String, Double> hPriorMap = new Hashtable<>();
              hPriorMap.put(strTypes[0], new Double(fPrior[0]));
              hPriorMap.put(strTypes[1], new Double(fPrior[1]));

              final DisambiguationRulesKey key =
                  new DisambiguationRulesKey(strTypes[0], strTypes[1]);
              final DisambiguationRulesOutcome outcome = m_Rules.Classify(key, strCtx, hPriorMap);
              if (m_FullDebug) {
                System.out.println(
                    "--> classified as: " + outcome.Class() + " (" + outcome.Likelihood() + ")");
              }
              ForceEntityTypeExt(i,
                  new NamedEntityType(hLabel2NEType.get(outcome.Class()), NETagSet.length,
                      new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.CLASSIFIER,
                          new Object[] {curTok.Raw(), hLabel2NEType.get(outcome.Class()).Label(),
                              outcome.Likelihood(), hPriorMap})));
              ...
              final CombinationGenerator cg = new CombinationGenerator(curTypes.TypeCount(), 2);
              final Hashtable<String, Double> hVote = new Hashtable<>();
              ...
              while (cg.hasMore()) {
                final int[] classPair = cg.getNext();
                // get prior for this entity
                final double[] fPrior = m_Prior.GetPrior(curTok.LexiconMatch(),
                    strTypes[classPair[0]], strTypes[classPair[1]]);
                hPriorMap.put(strTypes[classPair[0]], new Double(fPrior[0]));
                hPriorMap.put(strTypes[classPair[1]], new Double(fPrior[1]));

                final DisambiguationRulesKey key =
                    new DisambiguationRulesKey(strTypes[classPair[0]], strTypes[classPair[1]]);
                final DisambiguationRulesOutcome outcome = m_Rules.Classify(key, strCtx, hPriorMap);
                final Double fCurVote = hVote.get(outcome.Class());
                hVote.put(outcome.Class(),
                    new Double(fCurVote.doubleValue() + outcome.Likelihood()));
              }
              ...
              double fSum = 0.0f;
              for (int j = 0; j != strTypes.length; ++j) {
                final double fCurVote = hVote.get(strTypes[j]).doubleValue();
                fSum += fCurVote;
              ...
              if (m_FullDebug) {
                System.out.println("--> classified as: " + strTypes[nWinner]
                    + " (Probability sum = " + fMaxVote + ")");
              }
              ForceEntityTypeExt(i, new NamedEntityType(hLabel2NEType.get(strTypes[nWinner]),
                  NETagSet.length,
                  new NamedEntityExplanation(NamedEntityExplanation.ExplanationType.CLASSIFIER_VOTE,
                      new Object[] {curTok.Raw(), hLabel2NEType.get(strTypes[nWinner]).Label(),
                          fMaxVote, fSum, hPriorMap})));
  ...
  private void CheckVeryAmbiguousTypes() {
    ...
    for (int i = 0; i != m_TokenList.Size(); ++i) {
      ...
      if (tok.EntityType().IsStart()
          && tok.EntityType().Intersect(NamedEntityTypeEnum.HighlyAmbiguous())) {
        ...
        if (CountCapitalizedWords(i) > 1 || NERServices.OneWordEntity(tok.EntityType())
        ...
        final int nPreceding = i - 1;
        final int nFollowing = FindEndIndexNerf(i) + 1;
        if (nPreceding > 0 && nFollowing < m_TokenList.Size()) {
          final Token precTok = m_TokenList.Get(nPreceding);
          final Token nextTok = m_TokenList.Get(nFollowing);
          if (TokenFeature.Feature.HasOpenBracketPunct.Mechanism()
              .GetBooleanValue(precTok.Features())
              && TokenFeature.Feature.HasCloseBracketPunct.Mechanism()
                  .GetBooleanValue(nextTok.Features())
              || TokenFeature.Feature.HasOpenParenthesisPunct.Mechanism()
                  .GetBooleanValue(precTok.Features())
                  && TokenFeature.Feature.HasCloseParenthesisPunct.Mechanism()
                      .GetBooleanValue(nextTok.Features())) {
            if (m_FullDebug) {
        ...
        if (bDisable) {
          ForceEntityTypeExt(i,
              new NamedEntityType(NamedEntityTypeEnum.NOTHING, m_TagSetSize,
                  new NamedEntityExplanation(
                      NamedEntityExplanation.ExplanationType.REJECTED_VERY_AMBIGUOUS,
                      new Object[] {})));
          if (m_FullDebug) {
  ...
  private void PostCheckEntityBoundaryNerf() {
    ...
    for (int i = 0; i != m_TokenList.Size() - 1; ++i) {
      ...
      if (curTok.SentenceNumber() == nextTok.SentenceNumber() && curTok.EntityType().IsEnd()) {
        ...
        if (curTok.EntityType().Intersect(NamedEntityTypeEnum.Mergable()) && !bInsideAllCapitalized
            && TokenConsts.Is(nextTok.Type(), TokenConsts.TYPE_WORD)
            && nextTok.EntityType().HasNoTag() && TokenFeature.Feature.StartsWithCapital.Mechanism()
                .GetBooleanValue(nextTok.Features())) {
          if (m_FullDebug) {
            System.out.println("Entity + capitalized: " + curTok.Raw() + " | " + nextTok.Raw());
          }
          final NamedEntityType curType = curTok.EntityType()
              .clone(new NamedEntityExplanation(
                  NamedEntityExplanation.ExplanationType.ENTITY_CAPITAL_MERGING,
                  new Object[] {curTok.Raw(), nextTok.Raw()}));
        ...
        // a period follows any small word (e.g., middle word)
          ...
          if (curTok.SentenceNumber() == secondNextTok.SentenceNumber()) {
            ...
            final NamedEntityType curType = curTok.EntityType()
                .clone(new NamedEntityExplanation(
                    NamedEntityExplanation.ExplanationType.ENTITY_PUNCT_MERGING,
                    new Object[] {curTok.Raw(), nextTok.Raw()}));
            curType.RemoveType(NamedEntityTypeEnum.START, null);
            curType.RemoveType(NamedEntityTypeEnum.END, null);
            ExtendEntityExt(i, curTok, nextTok, curType);
        ...
        // a hyphen or a ampersand follows an organization name.
          ...
          if (curTok.EntityType().Intersect(NamedEntityTypeEnum.Mergable())
              && !bInsideAllCapitalized && curTok.SentenceNumber() == secondNextTok.SentenceNumber()
              && TokenFeature.Feature.StartsWithCapital.Mechanism()
                  .GetBooleanValue(secondNextTok.Features())
  ...
  private void CheckUnknownCapitalizedExt() {
    ...
    // check unknown words
    final Enumeration<String> eCur = m_UnlistedWords.keys();
    while (eCur.hasMoreElements()) {
      final String strUnknown = eCur.nextElement();
      final UnlistedWordInfo uwi = m_UnlistedWords.get(strUnknown);
      // annotate every capitalized occurences of this word according to
      // this type
      final ArrayList<Integer> alIndexes = m_CapWord2Index.get(strUnknown);
      if (alIndexes != null) {
        final Iterator<Integer> iCur = alIndexes.iterator();
        while (iCur.hasNext()) {
          final int idx = iCur.next().intValue();
          final Token curTok = m_TokenList.Get(idx);
          if (curTok.EntityType().HasNoTag()) {
            PromoteUnknownWordToEntity(curTok, uwi.Type());
            curTok.NamedEntityAlias(uwi.Group());
            if (m_FullDebug) {
              System.out.println("Unknown capitalized tagged: " + curTok.Raw() + "("
                  + curTok.EntityType().GetLabel(m_TokenList.NETagSet()) + ")");
            }
          }
        }
      }
  ...
  private void PromoteUnknownWordToEntity(final Token pi_Token, final NamedEntityType pi_Type) {
    final NamedEntityType newNE = new NamedEntityType(m_TagSetSize, new NamedEntityExplanation(
        NamedEntityExplanation.ExplanationType.UNKNOWN_CAPITAL, new Object[] {pi_Token.Raw()}));
  ...
  protected void ExtendEntityExt(final int pi_StartIndex, final Token pi_FirstBoundary,
      final Token pi_NewEnd, final NamedEntityType pi_NewType) {
    ...
    if (pi_NewEnd.Raw().length() > 4) {
      m_UnlistedWords.put(pi_NewEnd.Raw().toLowerCase(),
          new UnlistedWordInfo(pi_NewType, pi_FirstBoundary.NamedEntityAlias()));
    }
  }

  private void MergeEntitiesExt(final int pi_StartIndex, final Token pi_FirstBoundary,
      final Token pi_SecondBoundary, final NamedEntityType pi_NewType) {
    pi_FirstBoundary.EntityType().RemoveType(NamedEntityTypeEnum.END, null);
    pi_SecondBoundary.EntityType().RemoveType(NamedEntityTypeEnum.START, null);
    ForceEntityTypeExt(pi_StartIndex, pi_NewType);
  }

  private void ForceEntityTypeExt(final int pi_CurPosition, final NamedEntityType pi_EntityType) {
    final Token curTok = m_TokenList.Get(pi_CurPosition);

    // force previous tokens
    if (!curTok.EntityType().IsStart()) {
      int nPrev = pi_CurPosition - 1;
      while (nPrev > -1) {
        final Token prevTok = m_TokenList.Get(nPrev);
    ...
    if (!curTok.EntityType().IsEnd()) {
      while (nNext < m_TokenList.Size()) {
        final Token nextTok = m_TokenList.Get(nNext);
        boolean bIsEnd = false;
        if (nextTok.EntityType().IsStart()) {
          throw new Error("Inconsistant state");
        }
  ...
  private int FindEndIndexNerf(final int pi_Start) {
    int nEnd = pi_Start;
  ...
  @Override
  protected boolean IsEntityCounterExample(final NamedEntityType pi_EntityType) {
    return false;
  }
